...
devices_number = int(pytest.config.getoption('devices_number'))
...
repeats = devices_number / len(public_keys) if public_keys else 0
...
class TestChatBot(SingleDeviceTestCase):
    ...
    def test_one_to_one_chatbot(self, key):
        ...
        message = 'Message # %s, sent by e2e test'
        counter = 0
        while counter < messages_number:
            counter += 1
            # time.sleep(randint(60, 120))
            chat.chat_message_input.send_keys(message % counter)
            chat.send_message_button.click()

    @pytest.mark.parametrize('number', list(range(devices_number)))
    def test_chatbot_public_chat(self, number):
        ...
        counter = 0
        while counter <= messages_number / devices_number:
            counter += 1
