...
(defn get-removed-unseen-count
  [current-public-key user-statuses chat-id removed-messages-ids]
  (- (count removed-messages-ids)
     (count (filter (fn [[message-id statuses]]
                      (= :seen
                         (:status (get statuses
                                       current-public-key))))
                    user-statuses))))
...
(fx/defn clean-up-chats
  [{:keys [db] :as cofx} removed-messages-by-chat]
  (apply fx/merge cofx
...
(fx/defn block-contact
  [{:keys [db get-blocked-user-data] :as cofx} public-key]
  (let [contact (assoc (contact.db/public-key->contact
                        (:contacts/contacts db)
                        public-key)
                       :blocked? true)
        {:keys [messages chat]} (get-blocked-user-data public-key)
        removed-messages-by-chat (group-by :chat-id messages)]
    (fx/merge cofx
              {:db (-> db
